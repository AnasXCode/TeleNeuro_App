...
class _CalendarPageState extends State<CalendarPage> {
  ...
  Future<void> _submitRating(String appointmentId, String doctorId, double newRating, String reviewText) async {
    ...
    await firestore.runTransaction((transaction) async {
      ...
      double newAverage = ((oldRating * totalReviews) + newRating) / (totalReviews + 1);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.delete_sweep, color: Colors.white), onPressed: _clearHistory),
        ],
      ),
      body: user == null
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (status == 'Completed' && !isRated && !ratingExpired) {
                ...
                if (completedAt != null) {
                  int mins = DateTime.now().difference(completedAt.toDate()).inMinutes;
                  if (mins >= 5) {
                    ratingExpired = true;
                    FirebaseFirestore.instance.collection('appointments').doc(appointmentId).update({'ratingExpired': true});
                  } else {
                    showRateButton = true;
                  }
                } else {
                      ...
                      Padding(
                        ...
                        child: SizedBox(
                          ...
                          child: TextButton.icon(
                            onPressed: () => _showRatingDialog(context, appointmentId, data['doctorId'], data['doctorName']),
                            icon: const Icon(Icons.star_outline, color: Colors.amber),
                            label: const Text("Rate Your Visit (Limited Time)", style: TextStyle(color: Color(0xFF1565C0), fontWeight: FontWeight.bold)),
                          ),
                        ),
                      ),
                      ...
                      const Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            SizedBox(width: 5),
                            Text("Feedback Submitted", style: TextStyle(color: Colors.green, fontSize: 13, fontWeight: FontWeight.bold)),
                          ],
                        ),
                      ),
                      ...
                      const Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            SizedBox(width: 5),
                            Text("Rating Time Expired", style: TextStyle(color: Colors.grey, fontSize: 13, fontWeight: FontWeight.bold)),
                          ],
                        ),
                      ),
                  ],
